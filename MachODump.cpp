...
//===----------------------------------------------------------------------===//
//
// This file implements the MachO-specific dumper for llvm-rop.
//
//===----------------------------------------------------------------------===//
...
static const Target *GetTarget(const MachOObjectFile *MachOObj,
  ...
  *ThumbTarget = TargetRegistry::lookupTarget(ThumbTripleName, Error);
  ...
  errs() << "llvm-rop: error: unable to get target for '";
  if (!TheTarget)
    errs() << TripleName;
  else
...
static bool checkMachOAndArchFlags(ObjectFile *O, StringRef Filename) {
  if (isa<MachOObjectFile>(O) && !ArchAll && ArchFlags.size() != 0) {
    ...
    if (!ArchFound) {
      errs() << "llvm-rop: file: " + Filename + " does not contain "
...
static void ProcessMachO(StringRef Filename, MachOObjectFile *MachOOF,
  ...
  //       option like -r, -l, etc.  Or just the normal llvm-rop option
...
void llvm::ParseInputMachO(StringRef Filename) {
  ...
  for (unsigned i = 0; i < ArchFlags.size(); ++i) {
    if (ArchFlags[i] == "all") {
      ...
      if (!MachOObjectFile::isValidArch(ArchFlags[i])) {
        errs() << "llvm-rop: Unknown architecture named '" + ArchFlags[i] +
                      "'for the -arch option\n";
        return;
      }
    }
  ...
  if (std::error_code EC = BinaryOrErr.getError()) {
    errs() << "llvm-rop: '" << Filename << "': " << EC.message() << ".\n";
    return;
  }
      ...
      for (uint64_t Index = Start; Index < End; Index += Size) {
        ...
        if (gotInst) {
          ...
          if (Arch == Triple::x86_64 || Arch == Triple::x86) {
            ...
            errs() << "llvm-rop: warning: invalid instruction encoding\n";
            if (Size == 0)
...
void llvm::printMachOUnwindInfo(const MachOObjectFile *Obj) {
  ...
  for (const SectionRef &Section : Obj->sections()) {
    ...
    if (SectName == "__compact_unwind")
      ...
      outs() << "llvm-rop: warning: unhandled __eh_frame section\n";
  }
}
